...
module.exports = function(thorin, opt, storeObj, stripe) {
  ...
  if (opt.singleSubscription === true && !AccountModel.field[planId]) {
    let fieldOpt = {
      create: false,
      update: false,
      filter: false
  ...
  /*
  * Returns the active subscription of a given account, when singleSubscription is enabled.
  * */
  if (opt.singleSubscription === true) {
    AccountModel.method('getSubscription', function getActiveSubscription(options) {
