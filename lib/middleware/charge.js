...
module.exports = function(thorin, opt, stripe) {
   ...
   *  - storeCharges[] -> array of local database charges for the client (only when sync=true)
   *  OPTIONS:
          ...
          for (let i = 0; i < charges.length; i++) {
            ...
            toSave.push(() => {
              return chargeObj.save().then(() => {
                if (isNew) {
                  dbItems.push(chargeObj);
                }
              });
            });
          }
          return thorin.series(toSave).then(() => {
            intentObj.data('storeCharges', dbItems);
          });
        });
      }
