...
module.exports = function(thorin, opt, stripe) {
  ...
  dispatcher
    ...
    .input({
      stripe_token: dispatcher.validate('STRING').default(null),
    ...
    .use((intentObj, next) => {
      ...
      let stripeToken = intentObj.input('stripe_token'),
        ...
        stripeMetadata = intentObj.data('metadata'),
      ...
      // TODO: make it work with stripe#customer.create
      calls.push(() => {
      ...
      calls.push((stop) => {
        ...
        let trialDays = intentObj.data('trial_days'),
          hasTrial = false;
        if (typeof trialDays === 'undefined') {
          trialDays = targetPlanObj.get('trial_days');
          if (trialDays > 0) hasTrial = true;
        } else {
          hasTrial = true;
        }
        if (hasTrial) {
          ...
          if (trialDays === 0) {
            ...
            trialEndAt = Date.now() + trialDays * 60 * 60 * 24 * 1000;
            trialEndAt = Math.floor(trialEndAt / 1000);
          }
          if (trialEndAt) {
        ...
        return stripe.subscriptions.create(data).then((subscription) => {
          ...
          intentObj.data('stripeSubscription', subscription);
      ...
      if (opt.singleSubscription === true) {
        calls.push(() => {
      ...
      thorin.series(calls, (err) => {
        ...
        intentObj.data('subscription', currentSubscriptionObj);
        next();
      });
    });
  ...
  dispatcher
    ...
    .use((intentObj, next) => {
      ...
      let accountObj = intentObj.data('account'),
        ...
        currentSubscriptionObj = intentObj.data('currentSubscription') || intentObj.data('subscription'),
