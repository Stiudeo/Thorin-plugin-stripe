...
module.exports = function(thorin, opt, stripe) {
   ...
   * */
  dispatcher
    .addMiddleware('stripe.plan.create')
    .input({
    ...
    .use((intentObj, next) => {
      ...
      if (!accountObj) {
        logger.error(`stripe.plan.create: requires "account" to be present in the intent data.`);
      ...
      calls.push(() => {
        ...
        return stripe.customers.create({
          ...
          logger.debug(`stripe.plan.create: customer ${stripeCustomer.id} created for account ${accountObj.id}`, {
      ...
      calls.push((stop) => {
        ...
        return stripe.subscriptions.create(data).then((subscription) => {
          logger.info(`stripe.plan.create: subscription: ${subscription.id} created for account: ${accountObj.id} of plan: ${targetPlanObj.code}`, {
        ...
        return stripe.subscriptions
          ...
          .then((subscription) => {
            ...
            return currentSubscriptionObj.save().then(() => {
              logger.info(`stripe.plan.create: subscription ${currentSubscriptionObj.id} of account ${accountObj.id} updated to plan ${targetPlanObj.id}`, {
                tags: ['stripe.subscription.create']
              });
            });
          });
      ...
      calls.push(() => {
        ...
        return accountObj.save().then(() => {
          logger.info(`stripe.plan.create: account ${accountObj.id} upgraded to plan ${targetPlanObj.code} from ${currentPlanObj && currentPlanObj.code || "none"}`, {
            tags: ['stripe.upgrade']
          });
        });
      });
      ...
      thorin.series(calls, (err) => {
        if (err) {
          if(err.ns !== 'STRIPE') {
            logger.warn(`stripe.plan.create: could not complete subcription to plan ${targetPlanObj.code} of account ${accountObj.id} with token ${stripeToken}`);
            logger.debug(err);
          }
   ...
   * */
  dispatcher
    .addMiddleware('stripe.plan.cancel')
    .input({
    ...
    .use((intentObj, next) => {
      ...
      if(!accountObj) {
        logger.error(`stripe.plan.cancel: requires "account" to be present in the intent data.`);
        return next(thorin.error('STRIPE.HANDLE_ERROR', 'An error occurred while processing your request.', 400));
      }
      ...
      calls.push((stop) => {
        ...
        return stripe.subscriptions.del(currentSubscriptionKey, {
          ...
          logger.info(`stripe.plan.cancel: account ${accountObj.id} subscription ${currentSubscriptionObj.id} will be cancelled at period end.`, {
      ...
      thorin.series(calls, (err) => {
        if(err) {
          if(err.ns !== 'STRIPE') {
            logger.warn(`stripe.plan.cancel: could not complete downgrade from plan ${currentPlanObj.code} of account ${accountObj.id}`);
            logger.debug(err);
          }
          return next(thorin.error(err));
        }
